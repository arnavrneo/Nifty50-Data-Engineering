...
def pandas_preprocess(key):
    ...
    if "_sec_archives_processed.csv" in key:
        ...
        sec_arch["CH_TOT_TRADED_VAL"] = sec_arch["CH_TOT_TRADED_VAL"].apply(
        ...
        sec_arch.to_csv(
            f"/tmp/secarch/{key.split('_processed')[0]}_analytics.csv", index=False
        )
        ...
        short_selling_arch = short_selling_arch.rename(
            columns={
                "SS_NAME": "STOCK_NAME",
                "SS_SYMBOL": "SYMBOL",
                "SS_DATE": "TIMESTAMP",
        ...
        short_selling_arch.to_csv(
            f"/tmp/shortsellarch/{key.split('_processed')[0]}_analytics.csv",
            index=False,
        )
        ...
        nse_ratios = nse_ratios.rename(columns={"HistoricalDate": "TIMESTAMP"})

        nse_ratios.to_csv(
            f"/tmp/indexratios/{key.split('_processed')[0]}_analytics.csv", index=False
        )
        ...
        monthly_adv_dec = monthly_adv_dec.rename(
            columns={
        ...
        monthly_adv_dec.to_csv(
            f"/tmp/monthlyadvdec/{key.split('_processed')[0]}_analytics.csv",
            index=False,
        )
        ...
        bulk_deals_arch = bulk_deals_arch.rename(
            columns={
        ...
        bulk_deals_arch.to_csv(
            f"/tmp/bulkdealarch/{key.split('_processed')[0]}_analytics.csv", index=False
        )
        ...
        board_meetings = board_meetings.rename(
            columns={
                "bm_symbol": "SYMBOL",
                "bm_date": "TIMESTAMP",
                "bm_purpose": "PURPOSE",
        ...
        board_meetings.to_csv(
            f"/tmp/boardmeetings/{key.split('_processed')[0]}_analytics.csv",
            index=False,
        )
        ...
        block_deals_arch = block_deals_arch.rename(
            columns={
                "BD_DT_DATE": "TIMESTAMP",
                "BD_SYMBOL": "SYMBOL",
                "BD_SCRIP_NAME": "STOCK_NAME",
        ...
        block_deals_arch.to_csv(
            f"/tmp/blockdealarch/{key.split('_processed')[0]}_analytics.csv",
            index=False,
        )
