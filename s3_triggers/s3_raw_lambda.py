...
def pandas_preprocess(key):
    ...
    if "_sec_archives.csv" in key:
        ...
        sec_arch["CH_TIMESTAMP"] = pd.to_datetime(
            sec_arch["CH_TIMESTAMP"], format="%Y-%m-%d"
        )
        ...
        s3.upload_file(
            file_loc,
            dest_bucket,
            "secarch/" + processed_file,
            ExtraArgs={"ServerSideEncryption": "AES256"},
        )

    elif "_short_selling_archives.csv" in key:
        short_selling_arch = pd.read_csv("/tmp/shortsellarch/" + key)
        short_selling_arch = short_selling_arch.drop(
            ["_id", "TIMESTAMP", "createdAt", "updatedAt", "mTIMESTAMP", "__v"], axis=1
        )
        short_selling_arch["SS_DATE"] = pd.to_datetime(short_selling_arch["SS_DATE"])
        short_selling_arch.to_csv(
            f"/tmp/shortsellarch/{key.split('.')[0]}_processed.csv", index=False
        )
        ...
        nse_ratios.to_csv(
            f"/tmp/indexratios/{key.split('.')[0]}_processed.csv", index=False
        )
        ...
        s3.upload_file(
            file_loc,
            dest_bucket,
            "indexratios/" + processed_file,
            ExtraArgs={"ServerSideEncryption": "AES256"},
        )

    elif "_monthly_adv_declines.csv" in key:
        monthly_adv_dec = pd.read_csv("/tmp/monthlyadvdec/" + key)
        monthly_adv_dec = monthly_adv_dec.drop(
            ["_id", "TIMESTAMP", "ADM_MONTH", "ADM_REC_TY"], axis=1
        )
        monthly_adv_dec["ADM_MONTH_YEAR_STRING"] = pd.to_datetime(
            monthly_adv_dec["ADM_MONTH_YEAR_STRING"]
        )
        monthly_adv_dec.to_csv(
            f"/tmp/monthlyadvdec/{key.split('.')[0]}_processed.csv", index=False
        )
        ...
        bulk_deals_arch["BD_DT_DATE"] = pd.to_datetime(
            bulk_deals_arch["BD_DT_DATE"], format="%d-%b-%Y"
        )
        bulk_deals_arch.to_csv(
            f"/tmp/bulkdealarch/{key.split('.')[0]}_processed.csv", index=False
        )
        ...
        s3.upload_file(
            file_loc,
            dest_bucket,
            "bulkdealarch/" + processed_file,
            ExtraArgs={"ServerSideEncryption": "AES256"},
        )

    elif "_board_meetings.csv" in key:
        board_meetings = pd.read_csv("/tmp/boardmeetings/" + key)
        board_meetings = board_meetings.drop(
            ["sm_isin", "attachment", "diff", "sysTime", "bm_timestamp"], axis=1
        )
        board_meetings["bm_date"] = pd.to_datetime(board_meetings["bm_date"])
        board_meetings.to_csv(
            f"/tmp/boardmeetings/{key.split('.')[0]}_processed.csv", index=False
        )
        ...
        s3.upload_file(
            file_loc,
            dest_bucket,
            "boardmeetings/" + processed_file,
            ExtraArgs={"ServerSideEncryption": "AES256"},
        )

    elif "_block_deals_archives.csv" in key:
        block_deals_arch = pd.read_csv("/tmp/blockdealarch/" + key)
        block_deals_arch = block_deals_arch.drop(
            ["_id", "createdAt", "updatedAt", "mTIMESTAMP", "__v", "TIMESTAMP"], axis=1
        )
        block_deals_arch["BD_DT_DATE"] = pd.to_datetime(
            block_deals_arch["BD_DT_DATE"], format="%d-%b-%Y"
        )
        block_deals_arch.to_csv(
            f"/tmp/blockdealarch/{key.split('.')[0]}_processed.csv", index=False
        )
