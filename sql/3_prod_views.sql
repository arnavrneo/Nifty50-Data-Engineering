...
where timestamp = (select max(timestamp) from dev.block_deals)
...
select
    ...
    trading_price
from dev.bulk_deals
where timestamp = (select max(timestamp) from dev.bulk_deals)
order by qty_traded DESC
limit 10;
...
where timestamp = (select max(timestamp) from dev.sec_archives)
order by timestamp ASC;
