...
class NSEEquitiesData(Helper):
    ...
    def security_wise_archive(self, start_date, end_date, series="ALL"):
        ...
        for i in range(len(payload_all)):
            tmp_lst.append(payload_all[i][0])
...
def lambda_handler(event, context):
  ...
  ratios = indicesClass.get_nse_indices_ratios('Nifty 50', '12-May-2024', datetime.datetime.now().strftime('%d-%b-%Y'), 'Nifty 50')
  ratios.to_csv(f"nse_ratios_{current_time}.csv", index=False)

  # NSE Equities
  equitiesClass = NSEEquitiesData()
  print("[INFO] Fetching security archives...")
  sec_archives = equitiesClass.get_security_wise_archive("16-07-2024", current_time, "ALL")
  sec_archives.to_csv(f"sec_arch_{current_time}.csv", index=False)
  
  print("[INFO] Fetching bulk deals archives...")
  bulk_deals_archives = equitiesClass.get_bulk_deals_archives('16-07-2024', current_time)
  bulk_deals_archives.to_csv(f"bulk_deals_arch_{current_time}.csv", index=False)
  
  print("[INFO] Fetching block deals archives...")
  block_deals_archives = equitiesClass.get_block_deals_archives('16-07-2024', current_time)
  block_deals_archives.to_csv(f"block_deals_arch_{current_time}.csv", index=False)
  
  print("[INFO] Fetching short selling archives...")
  short_selling_archives = equitiesClass.get_short_selling_archives('16-07-2024', current_time)
  short_selling_archives.to_csv(f"short_selling_arch_{current_time}.csv", index=False)
  ...
  board_meetings = equitiesClass.get_board_meetings('16-07-2024', current_time)
  board_meetings.to_csv(f"board_meetings_{current_time}.csv", index=False)
